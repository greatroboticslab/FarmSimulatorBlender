...
You can use .bvh files generated by [MoMask](https://github.com/EricGuo5513/momask-codes) to create animations too. Open a new Blender project with the Unity humanoid skeleton. Then import the .bvh file. Now first select the .bvh skeleton, then shift-right-click the Unity skeleton so that it is yellow. Then open a new text editor window, open **convert_bvh.py**, and run it. The animations should now be converted to the Unity skeleton, and you can export it.